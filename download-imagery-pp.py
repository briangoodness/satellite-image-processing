...
# test thread
def test_thread_func(country, key, top_idx, bottom_idx, left_idx, right_idx, centroid_x_coords, centroid_y_coords, bands_data, print_int=50, max_images=24950):
    print('\bottom_idx: %s. max_images: %s\n' % (type(bottom_idx), type(max_images)))

# function to loop over cells in partition and download daytime imagery
def get_partition_imgs(country, key, top_idx, bottom_idx, left_idx, right_idx, centroid_x_coords, centroid_y_coords, bands_data, print_int=50, max_images=24950):
    m = 1
...
def help():
    print('\nusage:')
    print("use --country= for setting the country name (%s)" % country_list)
    ...
    print('use --bottomidx= for setting the bottom row to complete looping over the country.')
    print('(note: the top-left index should not be altered.)\n')

# run main
def main(argv):
    try:
        opts, args = getopt.getopt(argv, "h", ["help", "country="])
    ...
    if ('--help' in optlist) or ('-h' in optlist):
        help()
        sys.exit(2)
    elif '--country' not in optlist:
        print('\nSpecify a country using the --country argument, from one of the following countries: %s.\n\nSee help (-h or --help) for more information.\n' % country_list)
        sys.exit(2)
    for opt, arg in opts:
        if opt == '--country':
            country = arg
            print('\nCountry selected: %s' % country)
    ...
    # read the global nightlight image, retrieve the image boundaries, coordinates, and cell sizes
    ...
    # get partitions for each API key (split by row index)
    start_row_indices = map(int, open('data/google_image/%s-row-indices.txt' % country, 'r').read().splitlines())
    partitions = pd.DataFrame(start_row_indices, index=np.arange(1, len(start_row_indices) + 1), columns=['row_indices'])
    partitions['api_keys'] = partitions.index.map(lambda x: os.getenv('GMAP_API_KEY_%s' % x))
    #partitions = partitions[0:4]
    print(partitions)

    # Run each partition in separate threads
    t0 = time.time()
    threads = []
    for i, (start_row_index, api_key) in partitions.iterrows():
        func_args = {
                        'country':country,
                        'key':api_key,
                        'top_idx':start_row_index,
                        'bottom_idx':bottom_idx,
                        'left_idx':left_idx,
                        'right_idx':right_idx,
                        'centroid_x_coords':centroid_x_coords,
                        'centroid_y_coords':centroid_y_coords,
                        'bands_data':bands_data,
                        'print_int':50,
                        'max_images':71
                    }
        # thread = threading.Thread(target=get_partition_imgs, kwargs=func_args)
        # thread = multiprocessing.Process(target=get_partition_imgs, kwargs=func_args)
        thread = gevent.spawn(run=get_partition_imgs, **func_args)
